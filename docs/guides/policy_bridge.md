...
- 提供用于身体位置计算的正向运动学
- 面向多关节机构场景的前缀式消息过滤机制
- 同时支持位置目标（适用于位置执行器）与力矩目标（适用于电机 + PD 控制）。
...
12 自由度的四足策略，支持步态调节。提供预设步态：Trot（小跑）、Pronk（跳跃）、Bound（弹跳）和 Pace（溜蹄）。需要下载检查点文件(详见文件  `RoboJuDo/checkpoints/` README).
...
位置执行器更简单、更稳定。电机+PD 方式与训练动力学完全匹配，但需要配置增益。使用 GUI 中的复选框进行切换。
...
ZMQ 话题采用前缀匹配过滤机制（如 <前缀>/joint/…、<前缀>/control 等）。在多关节设备场景中，每个设备均以自身专属前缀进行消息的发布与订阅；Python 端会根据该前缀对接收的消息进行过滤，将状态数据路由至对应的策略实例。
