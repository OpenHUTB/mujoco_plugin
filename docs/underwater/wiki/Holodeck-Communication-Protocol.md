...
([`holodeck`](https://github.com/BYU-PCCL/holodeck) and
 [`holodeck-engine`](https://github.com/BYU-PCCL/holodeck-engine)) communicate.

This page is based off [these slides](protocol/protocol.pptx).


### Prerequisite Reading
Brush up on [semaphores](https://en.wikipedia.org/wiki/Semaphore_(programming))
and [shared memory](https://en.wikipedia.org/wiki/Shared_memory).

## The Two Halves of holodeck
...
`holodeck` and `holodeck-engine` needs to happen to make this example work:
...
## Part 1: `holodeck.make()`

The `holodeck.make()` function is mostly a helper function to instantiate a
`HolodeckEnvironment` object. `.make()` loads a configuration file and passes
the appropriate paramaters to the `__init__()` of `HolodeckEnvironment`.

The `__init__()` function does three main things:

1. Starts `holodeck-engine` process and tells it the minimum it needs to load
2. Creates HolodeckClient instance
...
![first diagram](protocol/loading-1.svg)
...
At this point of the `__init__()` of `HolodeckEnvironment`, it creates a 
...
- **Blocks the main game loop!**
  - The engine window will appear to be locked up while it is waiting on this 
    semaphore
...
2. The engine's default UUID is `""`. This means that if you launch the engine
   ...
   if you specify the UUID is `""` when creating the `HolodeckEnvironment` 
   object.